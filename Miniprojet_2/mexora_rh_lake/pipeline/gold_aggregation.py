...
def construire_gold(data_lake_root: str):
    ...
    silver_offres  = f"{data_lake_root}/silver/offres_clean/offres_clean.parquet".replace('\\', '/')
    silver_comp    = f"{data_lake_root}/silver/competences_extraites/competences.parquet".replace('\\', '/')
    ...
    df_top_comp = con.execute(f"""
        SELECT
            profil, famille, competence,
            COUNT(DISTINCT id_offre) AS nb_offres_mentionnent,
            ROUND(COUNT(DISTINCT id_offre) * 100.0 / (SELECT COUNT(DISTINCT id_offre) FROM '{silver_offres}'), 2) as pct_offres_total,
    ...
    df_salaires = con.execute(f"""
        SELECT
            ...
            ROUND(PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY salaire_median_mad) FILTER (WHERE salaire_connu), 0) AS salaire_q1_mad,
            ROUND(PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY salaire_median_mad) FILTER (WHERE salaire_connu), 0) AS salaire_q3_mad,
    ...
    df_villes = con.execute(f"""
        SELECT
            ...
            annee, mois,
            COUNT(*) AS nb_offres,
            COUNT(*) FILTER (WHERE teletravail ILIKE '%télétravail%' OR teletravail ILIKE '%remote%' OR teletravail ILIKE '%hybride%') AS nb_offres_remote,
            ROUND(COUNT(*) FILTER (WHERE teletravail ILIKE '%télétravail%' OR teletravail ILIKE '%remote%' OR teletravail ILIKE '%hybride%') * 100.0 / NULLIF(COUNT(*), 0), 1) AS pct_remote
        FROM '{silver_offres}'
    ...
    df_entreprises = con.execute(f"""
        SELECT
            ...
            LIST(DISTINCT profil_normalise ORDER BY profil_normalise) AS profils_recrutes,
    ...
    df_tendances = con.execute(f"""
        SELECT
            ...
            LAG(COUNT(*)) OVER (PARTITION BY profil_normalise ORDER BY annee, mois) AS nb_offres_mois_precedent
        FROM '{silver_offres}'
